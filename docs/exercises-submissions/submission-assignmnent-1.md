...
At the business level, the platform offers three connected capabilities. First, it controls who may participate through a moderated onboarding process. Second, it creates verifiable trust claims for content through peer-based review and attestation. Third, it provides a governance process for contesting and removing problematic content. Together, these capabilities form a coherent lifecycle from identity, to verification, to moderation. The architectural proposition is that this lifecycle is best implemented through a hybrid design: BPMN orchestration for stateful business flows, and Kafka-based eventing for loosely coupled side effects such as notifications.

### 1.2 Domain Structure and Bounded Contexts

The system is organised into distinct bounded contexts rather than one monolithic application. This is important both conceptually and technically. The user domain is responsible for onboarding and registration status, the verification domain is responsible for peer-based trust decisions, the reporting domain governs moderation and deletion, the attestation domain stores signatures, and the notification domain reacts to published events. This separation allows each part of the platform to retain a clear responsibility while still participating in end-to-end processes.
...
This decomposition is one of the key reasons the project remains understandable despite spanning several distributed concerns. Each process belongs to one service, and each service owns its own local state and task types. At the same time, the boundaries are deliberately porous through carefully chosen integration styles: synchronous REST where an immediate answer is required, Zeebe orchestration where explicit process state is needed, and Kafka where decoupled event distribution is preferable.
...
From a technical perspective, the process already demonstrates the overall style of the platform. The `user-service` exposes the external API, starts the BPMN process, executes the service tasks through a Zeebe worker, and persists its domain-facing state in an in-memory store. The generated `userId` then becomes the stable identifier used by the verification flow. In other words, this onboarding process does not stand alone; it establishes the identity foundation on which later trust decisions depend.
...
Technically, the `verification-service` is the most orchestration-heavy service in the system. It combines API handling, Zeebe process starts, worker execution, HTTP calls to other services, and Kafka publishing. Its central process variable `verificationId` is both a client-visible identifier and the correlation key for Zeebe messages, which keeps the API contract and the workflow state aligned. The process also demonstrates why the platform uses Camunda 8 rather than plain event choreography: peer review is long-running, externally driven, and failure-sensitive, so the system needs explicit process state rather than a chain of stateless consumers.
...
The five services are implemented in Python 3.12 with Flask. The three orchestrated services connect to Zeebe through `pyzeebe`, an asyncio-native Python library that supports process execution and job-worker behaviour. Each of them runs a Zeebe worker in a dedicated background loop and exposes an HTTP API for starting or interacting with process instances. The `attestation-service` is deliberately simpler: it is a REST-only service that stores and invalidates signatures. The `notification-service` has the opposite profile: it exposes no HTTP interface at all and exists purely as a Kafka consumer that materialises user-facing notification side effects through log output.

At the current project scale, all service-local state is stored in memory, which keeps the implementation lightweight and easy to understand for a course project. That decision also makes some limitations explicit and therefore educational: a service restart clears local read-side state, while the authoritative workflow state remains in Zeebe. This contrast highlights the difference between local service memory and durable process state, which is one of the central architectural ideas explored throughout the project.

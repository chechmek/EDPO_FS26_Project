...
For an overview of responsibilities across all exercises, see `[submission-responsibilities.md](./submission-responsibilities.md)`.

---

## Report on Stateful Resilience Patterns in the Software Project

For our content verification platform we implemented two of the stateful resilience patterns introduced in the lecture and Lab08 (Flowing Retail): **Stateful Retry** and **Human Intervention**. Both patterns are modelled directly in BPMN and executed by Camunda Zeebe. Because Zeebe is an **event-sourced workflow engine**, all changes to process state are recorded in an append-only event log and materialized into RocksDB snapshots (Lecture 6, slides 4-5). The current state of any process instance can be fully reconstructed from this log, meaning process variables survive crashes and redeployments without any additional persistence code on our side (Lecture 6, slide 8). This is the foundational property that makes stateful resilience patterns practical. Zeebe does not implement ACID transactions across services. As discussed in class, this presents two design options: commit to the domain first and then notify Zeebe (at-least-once), or notify Zeebe first and then commit (at-most-once) (Lecture 6, slide 25). We follow the **at-least-once** approach, committing domain state before completing the Zeebe job, accepting the possibility of duplicate processing in exchange for the guarantee that no state update is ever silently lost. Our patterns therefore rely on **state management and eventual consistency** rather than distributed transactions.
